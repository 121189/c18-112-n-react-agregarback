...
module.exports.searchRecipes = async (req, res) => {
    try {
        const { ingredientsQty, portionsQty, maxDuration, orderBy, order, keywords } = req.body;
        let filter = {};
        let sort = {
            createdAt: -1,
        };
...
module.exports.getFavorites = async (req, res) => {
    try {
        ...
        if (!user.favorites || user.favorites.length === 0) {
            res.status(200).json({ recipes: [], message: "No tienes recetas favoritas" });
            return;
        }
        const recipes = await Recipe.find();
        const favorites = recipes.filter(recipe => user.favorites.includes(recipe._id.toString()));
